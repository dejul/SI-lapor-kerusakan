! function (i, r, c) {
	...
	if (c(i).scroll(function () {
			...
			c("nav.header-navbar").addClass(e), setTimeout(function () {
				c(".horizontal-layout div.header-navbar").addClass(e)
			}, 10), 0 < c("body.horizontal-layout.vertical-overlay-menu.layout-dark:not(.layout-transparent)").length && c(".app-sidebar").data("background-color", "black");
			var a = !1;
			u.hasClass("nav-collapsed") && (a = !0), c("html").data("textdirection"), setTimeout(function () {
				d.removeClass("loading").addClass("loaded")
			}, 1200), c.app.menu.init(a);
			!1 === c.app.nav.initialized && c.app.nav.init({
				speed: 300
			}), Unison.on("change", function (e) {
				c.app.menu.change()
			}), 0 !== h.length && void 0 !== p && h.css("background-image", 'url("' + p + '")'), c('[data-toggle="tooltip"]').tooltip({
				container: "body"
			...
			c(".nav-link-search").on("click", function () {
				...
				e.hasClass("open") && (e.removeClass("open"), c(".search-input input").val(""), c(".search-input input").blur(), c(".search-input .search-list").removeClass("show"), c(".wrapper").hasClass("show-overlay") && c(".wrapper").removeClass("show-overlay"))
				...
				if (38 !== e.keyCode && 40 !== e.keyCode && 13 !== e.keyCode) {
					27 == e.keyCode && (c(".search-input input").val(""), c(".search-input input").blur(), c(".search-input").removeClass("open"), c(".search-list").hasClass("show") && (c(this).removeClass("show"), c(".search-input").removeClass("show")));
					...
					if (c("ul.search-list li").remove(), "" != n) {
						...
						c.getJSON("../app-assets/data/" + l + ".json", function (e) {
							for (var a = 0; a < e.listItems.length; a++)(0 == e.listItems[a].name.toLowerCase().indexOf(n) && o < 10 || 0 != e.listItems[a].name.toLowerCase().indexOf(n) && -1 < e.listItems[a].name.toLowerCase().indexOf(n) && o < 10) && (t += '<li class="auto-suggestion d-flex align-items-center justify-content-between cursor-pointer ' + (0 === o ? "current_item" : "") + '"><a class="d-flex align-items-center justify-content-between w-100" href=' + e.listItems[a].url + '><div class="d-flex align-items-center justify-content-start"><span class="mr-2 ' + e.listItems[a].icon + '"></span><span>' + e.listItems[a].name + "</span></div>", o++);
							"" == t && "" == s && (s = '<li class="auto-suggestion d-flex align-items-center cursor-pointer"><a class="d-flex align-items-center justify-content-between w-100"><div class="d-flex align-items-center justify-content-start"><span class="mr-2"></span><span>No results found.</span></div></a></li>'), i = t.concat(s), c("ul.search-list").html(i)
						})
			...
			var a, n, t = c(".search-list li.current_item");
			if (40 === e.keyCode ? (a = t.next(), t.removeClass("current_item"), t = a.addClass("current_item")) : 38 === e.keyCode && (n = t.prev(), t.removeClass("current_item"), t = n.addClass("current_item")), 13 === e.keyCode && 0 < c(".search-list li.current_item").length) {
		...
		var a = r.querySelector(".drag-target"),
			n = "panright",
			t = "panleft";
		if (!0 === e && (n = "panleft", t = "panright"), 0 < c(a).length) new Hammer(a).on(n, function (e) {
			if (u.hasClass("vertical-overlay-menu")) return c.app.menu.open(), !1
		});
		setTimeout(function () {
			var e, a = r.querySelector(".app-sidebar");
			0 < c(a).length && ((e = new Hammer(a)).get("pan").set({
		...
		if (0 < c(s).length) new Hammer(s).on(t, function (e) {
			if (u.hasClass("vertical-overlay-menu")) return c.app.menu.hide(), !1
		})
	}
	c(r).on("click", ".nav-toggle, .menu-toggle, .nav-close", function (e) {
		e.preventDefault(), c.app.menu.toggle(), setTimeout(function () {
			c(i).trigger("resize")
		}, 200);
		var a = c(".cz-compact-menu");
		return 0 < a.length && setTimeout(function () {
			u.hasClass("menu-expanded") || u.hasClass("menu-open") ? a.prop("checked", !1) : a.prop("checked", !0)
		}, 100), c(".vertical-overlay-menu .navbar-with-menu .navbar-container .navbar-collapse").hasClass("show") && c(".vertical-overlay-menu .navbar-with-menu .navbar-container .navbar-collapse").removeClass("show"), !1
	...
	}), "undefined" != typeof screenfull && screenfull.isEnabled && c(r).on(screenfull.raw.fullscreenchange, function () {
		...
		var e = c(this),
			a = e.attr("href"),
			n = e.closest(".nav");
		n.find(".nav-link").removeClass("active"), e.closest(".nav-item").find(".nav-link").addClass("active"), e.closest(".dropdown-menu").find(".dropdown-item").removeClass("active"), e.addClass("active"), n.next().find(a).siblings(".tab-pane").removeClass("active in").attr("aria-expanded", !1), c(a).addClass("active in").attr("aria-expanded", "true")
		...
		c("#dropdown-flag .selected-flag").attr("src", t), i18next.changeLanguage(n, function (e, a) {
	...
	if (Array.prototype.forEach) {
		...
		for (o = 0; o < m.length; o++) m[o].data("size"), m[o].data("color"), new Switchery(m[o], {
			color: "#975AFF"
		})
	}
